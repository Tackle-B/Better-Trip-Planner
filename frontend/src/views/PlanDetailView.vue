            ...
            <a-list-item>
              <a-card size="small" class="weather-card">
                ...
                <div class="weather-info-row">
                  <span class="weather-icon">白天</span>
                  <div>
                ...
                <div class="weather-info-row">
                  <span class="weather-icon">夜间</span>
                  <div>
                ...
                <div class="weather-wind">
                  风向: {{ item.wind_direction }} {{ item.wind_power }}
                </div>
              </a-card>
            </a-list-item>
          </template>
...
import type { TripPlan, SavedTripPlan } from '@/types'
import * as api from '@/services/api'

const router = useRouter()
const route = useRoute()
const tripPlan = ref<TripPlan | null>(null)
const savedPlan = ref<SavedTripPlan | null>(null)
...
onMounted(async () => {
  ...
  if (planId) {
    await loadPlanFromAPI(planId)
  } else {
    // 兼容旧的 sessionStorage 方式
...
const loadPlanFromAPI = async (planId: string) => {
  try {
    message.loading({ content: '加载计划中...', key: 'load', duration: 0 })
    const response = await api.getTripPlan(planId)
    if (response.success && response.data) {
      savedPlan.value = response.data
      tripPlan.value = response.data.plan
      message.success({ content: '计划加载成功', key: 'load' })
    } else {
      throw new Error('计划不存在')
    }
  } catch (error: any) {
    console.error('加载计划失败:', error)
    message.error({ content: '加载计划失败: ' + error.message, key: 'load' })
    router.push('/chat')
  }
}
...
const saveChanges = async () => {
  if (!tripPlan.value || !savedPlan.value) {
    message.error('无法保存：计划数据不存在')
    return
  }
  ...
  try {
    ...
    // 调用 API 更新计划
    await api.updateTripPlan(savedPlan.value.plan_id, {
      plan: tripPlan.value
    })

    editMode.value = false
    message.success({ content: '修改已保存', key: 'save' })
